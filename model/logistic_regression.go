...
func (lr *LogisticRegression) Train(rate, c float64, features [][]float64, labels []float64) {
	for j := 0; j < len(features[0]); j++ {
		...
		lr.theta[j] -= rate * (total/float64(len(features)) + c*lr.theta[j]/float64(len(features)))
	}
}
